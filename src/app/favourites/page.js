...
export default function FavouritesPage() {
  ...
  const favouritesSectionRef = useRef(null);
    ...
    async (requestedPage, signal) => {
      ...
      try {
        const response = await fetch(
          `/api/my-favourites?page=${requestedPage}`,
          { cache: "no-store", signal }
        );
        const data = await response.json();

        if (!response.ok || data.success === false) {
          throw new Error(data.message || t("favouritesUnavailable"));
        }
        ...
        if (requestSequence !== requestSequenceRef.current) return;
        setError(t("favouritesUnavailable"));
      } finally {
  ...
  useEffect(() => {
    ...
    async function loadPage() {
      await loadFavourites(page, controller.signal);
    }

    loadPage();
    return () => controller.abort();
  }, [status, page, loadFavourites]);

  const changePage = (nextPage) => {
    if (nextPage === page) return;
    setPage(nextPage);
    setIsPageLoading(true);
    favouritesSectionRef.current?.scrollIntoView({
      behavior: "smooth",
      block: "start",
    });
  };
  ...
  const handleRemove = async (attractionId, attractionName) => {
    ...
    try {
      const response = await fetch(
        `/api/collection/favourites?attractionId=${attractionId}`,
        { method: "DELETE" }
      );
      ...
      if (data.success) {
        ...
        showToast(`${t("remove")}: ${attractionName || ""}`, "success");
      } else {
          ...
          <div className="w-full max-w-md rounded-[18px] bg-white p-6 shadow-2xl">
            ...
            <div className="mt-6 flex gap-3">
              <button
                ...
                className="flex-1 rounded-lg border border-[#BBC8D0] px-5 py-2.5 font-semibold text-[#004638]"
              >
                {t("cancel")}
              </button>
              <button
                ...
                disabled={isRemoving}
                className="flex-1 rounded-lg bg-[#C2413B] px-5 py-2.5 font-semibold text-white disabled:opacity-50"
              >
      ...
      <div className="bg-[#006C56] px-4 py-12 text-white">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-2 text-white/80">
            {totalFavourites === 0
              ? t("emptyFavourites")
              : t("attractionsAvailable", { count: totalFavourites })}
          </p>
        </div>
      </div>

      <div
        ref={favouritesSectionRef}
        ...
        {favourites.length === 0 ? (
          ...
          <>
              ...
              {favourites.map((item) => (
                ...
                <div className="p-4">
                  ...
                  <button
                    ...
                    className="mt-3 w-full rounded-lg border border-red-200 py-2 text-sm text-red-500 hover:bg-red-50"
                  >
                    {t("remove")}
                  </button>
                </div>
            ...
            <Pagination
              ...
              ariaLabel={t("favouritesPagination")}
              getPageAriaLabel={(pageNumber) =>
                t("favouritesGoToPage", { page: pageNumber })
              }
